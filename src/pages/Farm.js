...
const CryptoFarming = () => {
  const FARM_DURATION = 86400; // 24 hours in seconds

  const [timeRemaining, setTimeRemaining] = useState(FARM_DURATION);
  const TRIGGER_TIME = FARM_DURATION - 24;
  const TRIGGER_TIMETWO = FARM_DURATION - 24;
  const TRIGGER_TIMETHREE = FARM_DURATION - 24;
  const TRIGGER_TIMEFOUR = FARM_DURATION - 24;
  ...
  useEffect(() => {
    ...
    if (farmingDone === 'true') {
      ...
      const elapsedTime = Math.floor((Date.now() - parseInt(farmingStart)) / 1000); // Calculate elapsed time in seconds
      if (elapsedTime < FARM_DURATION) {
        setTimeRemaining(FARM_DURATION - elapsedTime); // Set the remaining time correctly
        setIsFarming(true);
        setPointsEarned((elapsedTime / FARM_DURATION) * POINTS_TO_EARN); // Calculate points earned based on elapsed time
      } else {
        completeFarming();
      }
    }
  ...
  useEffect(() => {
    ...
    if (isFarming && timeRemaining > 0) {
      interval = setInterval(() => {
        ...
        // Check if the farming is complete
        if (timeRemaining <= 0) {
          clearInterval(interval);
          completeFarming(); // Call the completion logic only when the timer hits zero
        }
  ...
  const startFarming = () => {
    ...
    localStorage.setItem('farmingStart', currentTime); // Store the farming start time
  ...
  const completeFarming = () => {
    ...
    setFarmingCompleted(true);
    localStorage.removeItem('farmingStart');
    localStorage.setItem('farmingCompleted', 'true'); // Only set this when farming is complete
  };
  ...
  return (
    <Animate>
        ...
        <div id="refer" className="w-full h-screen pt-5 scroller overflow-y-auto pb-[150px] space-y-3">
          ...
          <div className="w-full flex justify-center items-center text-center">
            <div className="w-[150px] h-[150px] fanbg border-[#616161] border-[4px] flex justify-center rounded-full items-center text-center relative">
              ...
              />
              <div className="absolute z-10 bg-[#3f2900] border-[1px] border-[#8b8b8b] rounded-full h-[64px] w-[64px] flex justify-center items-center">
                <img src="/dogs.webp" alt="sdfd" className="w-[54px]" />
              </div>
            </div>
          </div>
          ...
          <div className="w-full px-3 pt-5">
            ...
            <div className="w-full flex justify-center pt-5">
              <button
                ...
                className="w-[48%] px-4 py-3 flex items-center justify-center text-center rounded-[8px] bg-btn hover:bg-[#f3bf25a4] font-semibold text-[14px]"
              >
                Back to the game
              </button>
            </div>
            ...
            <div className="w-full flex pt-6 flex-col space-y-6 overflow-y-auto pb-[100px] scroller">
              <div className="flex items-center space-x-4">
                <div className="w-full">
                  <h1 className="font-semibold text-[24px] text-center pb-4">Settings</h1>
      ...
      >
        <div
          ref={modalRef}
          className={`${claimModal === true ? 'opacity-100 mt-0 ease-in duration-300' : 'opacity-0 mt-[100px]'} w-full bg-modal relative rounded-[16px] flex flex-col justify-center p-8`}
        >
          <div className="w-full flex justify-center flex-col items-center space-y-3">
            ...
            <span className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2 flex flex-col justify-center w-full text-center items-center space-x-1">
              <span className="flex items-center justify-center space-x-[2px] text-[18px]">
                ...
                <span className="text-accent pr-[6px]">+{formatNumber(pointsEarned)} Marble </span> CLAIMED
              </span>
            </span>
            <p className="pb-6 text-[15px] w-full text-center">Continue mining to claim more tokens daily! 😎</p>
          </div>
      ...
      >
        ...
        >
          <div className="w-full flex justify-center flex-col items-center space-y-3">
            ...
            <h3 className="font-medium text-center text-[20px] text-[#ffffff] pb-2 uppercase">{miningPower} PPH</h3>
            <p className="pb-6 text-[13px] w-full text-center">
              This is the amount of tokens you earn every 1 hour when mining is active, you can boost your mining profit
              by clicking on the Boost Mining option.
            </p>
          </div>
