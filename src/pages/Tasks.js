        ...
        <div className="w-full pt-1 justify-center flex-col space-y-6 px-5">
          ...
          <div className="w-full flex items-center justify-between border-cards border-[1px] p-1 rounded-[8px]">
            <div
              ...
              className={`${activeIndex === 1 ? 'bg-cards text-[#ebebeb]' : ''}  rounded-[6px] text-primary py-[10px] px-3 w-[33%] flex space-x-1 justify-center text-center text-[14px] font-semibold items-center`}
            >
            ...
            <div
              ...
              className={`${activeIndex === 2 ? 'bg-cards text-[#ebebeb]' : ''}  rounded-[6px] text-primary py-[10px] px-3 w-[33%] space-x-1 font-semibold text-[14px] flex justify-center text-center items-center`}
            >
          ...
          <div id="refer" className="w-full h-[80vh] !mt-3 scroller rounded-[10px] overflow-y-auto pt-2 pb-[250px]">
            {/* tasks */}
            <div
              className={`${activeIndex === 1 ? 'block' : 'hidden'} w-full flex items-end justify-center flex-col space-y-4`}
            >
              <div className={`px-1`}>
                <h1 className="text-[18px] font-semibold -mt-2">Social Tasks</h1>
                <p className="text-[14px] text-[#c6c6c6] leading-[24px] pr-8">
                  Perform social tasks to earn more Marble tokens and stay updated!
                </p>
              </div>
                ...
                >
                  <div className="flex h-full w-full flex-col bg-[#2d2d2d] justify-center rounded-[24px] py-4 pl-12 pr-4 relative">
                    ...
                    <div className="flex w-full flex-col justify-between h-full space-y-2">
                      ...
                      <h1 className="text-[15px] text-nowrap line-clamp-1 mr-[5px] font-medium">{task.title}</h1>
                      <span className="flex text-primary items-center w-fit space-x-1 text-[14px] font-semibold">
                        <span className={`w-[10px] h-[10px] bg-btn4 rounded-full flex items-center`}></span>
                          ...
                          <>
                            {task.verified && countdownFinished[task.id] && !completedTasks.includes(task.id) ? (
                              ...
                              currentError[task.id] && (
                                <span className={`text-accent pt-2 text-xs w-full`}>{currentError[task.id]}</span>
                              )
                            )}
                            {completedTasks.includes(task.id) && (
                              <>
                      ...
                      {claimError && <p className={`text-accent pt-2 text-xs w-full`}>{claimError}</p>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div
              className={`${activeIndex === 2 ? 'block' : 'hidden'} w-full flex items-end justify-center flex-col space-y-4`}
            >
              <div className={`px-1`}>
                <h1 className="text-[18px] font-semibold -mt-2">Watch & earn</h1>
                <p className="text-[14px] text-[#c6c6c6] leading-[24px] pr-8">
            ...
            <div
              className={`${activeIndex === 3 ? 'block' : 'hidden'} w-full flex items-end justify-center flex-col space-y-4`}
            >
              <div className={`px-1`}>
                <h1 className="text-[18px] font-semibold -mt-2">Invite friends, get rewards!</h1>
            ...
            >
              <div className="w-full flex justify-center flex-col items-center space-y-3">
                ...
                <h3 className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2">
                  <span className={`text-accent`}>+{formatNumberCliam(claimedBonus)}</span> Marble CLAIMED
                </h3>
                <p className="pb-6 text-[#9a96a6] text-[15px] w-full text-center">
                  Keep performing new tasks! something huge is coming! Perform more and earn more Marble now!
                </p>
              </div>
