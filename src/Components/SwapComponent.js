...
const SwapComponent = ({openSwapModal, setOpenSwapModal}) => {
    const {balance,profitHour, id, setBalance, setSwaps, walletAssets, setWalletAssets } = useUser();
    ...
    const [selectedAssetFrom, setSelectedAssetFrom] = useState(walletAssets.find(asset => asset.symbol === 'Marble'));
    ...
    const handleSwap = () => {
      ...
      const confirmSwap = async () => {
        ...
        const amountAfterFee = swapAmount - swapFee;
        const equivalentValue = amountAfterFee * (selectedAssetFrom.price / selectedAssetTo.price);
      
        let newBalance = balance;
        let newProfitHour = profitHour; // Track profitHour if Marble is selected
      
        const updatedAssets = walletAssets.map(asset => {
          if (asset.symbol === selectedAssetFrom.symbol) {
            if (selectedAssetFrom.symbol === 'Marble') {
              newProfitHour -= parseFloat(swapAmount); // Deduct from profitHour
            } else {
              newBalance -= parseFloat(swapAmount); // Deduct from balance for other assets
            }
            return {
              ...asset,
          ...
          if (asset.symbol === selectedAssetTo.symbol) {
            return {
              ...asset,
              balance: asset.balance + equivalentValue,
            };
          }
          return asset;
        });
        ...
        try {
          await updateDoc(userRef, {
            ...
            ...(selectedAssetFrom.symbol === 'Marble' && { profitHour: newProfitHour }),
            'history.swaps': arrayUnion(swapHistory),
          });
      
          setWalletAssets(updatedAssets);
          if (selectedAssetFrom.symbol === 'Marble') {
            setBalance(newProfitHour); // Set new profitHour balance for Marble
          } else {
            setBalance(newBalance); // Set new balance for other assets
          }
          ...
          setTimeout(() => {
            ...
            setSwapAmount('');
            setSelectedAssetFrom(walletAssets.find(asset => asset.symbol === 'Marble'));
    ...
    const handleSwapAmountChange = (e) => {
      ...
      // Check for balance error using profitHour for Marble
      if (selectedAssetFrom.symbol === 'Marble') {
        setBalanceError(amount > profitHour);
      } else if (selectedAssetFrom && amount > selectedAssetFrom.balance) {
        setBalanceError(true);
      } else {
        setBalanceError(false);
      }
      ...
      const handleAssetSelection = (asset, type) => {
        if (type === 'from') {
          ...
          // Automatically select the first asset in the list that's not Marble as the 'to' asset
          if (asset.symbol === 'Marble') {
            const filteredAssets = walletAssets.filter(asset => asset.symbol !== 'Marble');
            setSelectedAssetTo(filteredAssets[0]); // Select the first non-Marble asset by default
          }
        } else if (type === 'to') {
          // Ensure Marble is not selectable as 'to' asset
          if (asset.symbol !== 'Marble') {
      ...
      ? walletAssets.filter(asset => asset.symbol !== 'Marble') 
      : walletAssets;
      ...
      const cancelSwap = () => {
        ...
        setSwapAmount('');
        setCalculatedSwapValue(0);
        setSelectedAssetFrom(walletAssets.find(asset => asset.symbol === 'Marble')); // Reset to default 'from' asset
...
<div className="flex justify-between items-center">
  ...
  {selectedAssetFrom && (
                <span className="text-secondary text-[11px]">
                    Available: {formatNumber(selectedAssetFrom.profitHour)} {selectedAssetFrom.symbol}
                  </span>
                )}
